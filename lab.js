...
(function () {
  ...
  function runJourney(segments, o = {}) {
    ...
    const apply = async (seg, fade) => { for (const id of union) { const m = seg.mix.find(x => x.id === id); if (m) { if (m.params) engine.setSculpt(m.params, id); if (!engine.isActive(id)) { await engine.startSound(id, 0.001); if (journeyTok !== me) { if (engine.isActive(id)) engine.stopSound(id); return; } } /* a start that lands after Stop is undone, not ramped */ engine.rampVolume(id, m.volume, fade); } else if (engine.isActive(id)) engine.rampVolume(id, 0.0001, fade); } if (seg.visual && o.visuals && seg.visual !== focus.currentVisual()) focus.crossfadeTo(seg.visual); if (seg.tone !== undefined) engine.setMasterTone(seg.tone, fade); if (o.visuals) { for (const key of ['dim', 'slow', 'time']) if (seg[key] !== undefined) rampParam(key, seg[key], fade); } };
  ...
  const EXPERIMENTS = [
    ...
    },
    {
      ...
      buildUI(ctx, host) {
        host.innerHTML = `<p class="muted" style="margin:0 0 8px">Paint the sound you want to hear. Left is low, right is high; higher means more.</p><p class="paint-compare-note" data-compare-note hidden>Now hearing: your preferred sound — tap “Back to my painting” to edit again.</p><canvas class="paint-canvas" width="900" height="300" aria-label="Frequency painting canvas. Draw to shape the sound."></canvas><div class="scale"><span>60 Hz</span><span>500</span><span>2k</span><span>6k</span><span>14 kHz</span></div>
          ...
          <details class="kbd-alt" open><summary class="muted small">Adjust without dragging</summary><div class="kbd-grid">${[60, 120, 250, 500, 1000, 2000, 4000, 8000, 14000].map((f, i) => `<label>${f >= 1000 ? f / 1000 + 'k' : f}<input type="range" min="0" max="100" data-band="${Math.round(i * 23 / 8)}" aria-label="Band ${f} hertz"></label>`).join('')}</div></details><div data-saveform></div><div class="lab-saved" data-saved></div>`;
        ...
        // Compare: the painting can't be heard, so it is dimmed, labelled and not editable until "Back to my painting"
        let comparing = false; const EDITS = ['paint', 'reduce', 'smooth', 'undo', 'reset', 'random', 'preview'];
        const setCompare = (on) => {
          comparing = on; c.classList.toggle('is-comparing', on); const note = $('[data-compare-note]', host); if (note) note.hidden = !on;
          const cb = $('[data-act="compare"]', host); if (cb) { cb.setAttribute('aria-pressed', on); cb.textContent = on ? 'Back to my painting' : 'Compare with my preferred sound'; }
          $$('[data-act]', host).forEach(x => { if (EDITS.includes(x.dataset.act)) x.disabled = on; }); $$('[data-band]', host).forEach(r => { r.disabled = on; });
          if (!on) syncUndo();
        };
        engine.on(type => { if (type === 'sounds' && comparing && c.isConnected && !engine.isActive('sculpt')) setCompare(false); });   // the preferred sound stopped (Stop, player): back to normal
        c.addEventListener('pointerdown', e => { if (comparing) return; down = true; push(); c.setPointerCapture(e.pointerId); paint(e); }); c.addEventListener('pointermove', e => { if (down) paint(e); }); addEventListener('pointerup', () => { if (down) store.set('lab:paintcurve', ctx.curve); down = false; });
    ...
    },
    {
      ...
      buildUI(ctx, host) {
        ...
        $('[data-journey]', host).addEventListener('click', () => {
          ...
          app.armIntent('lab:toJourney', () => { if (!btn.isConnected) return; if (running) stopRunning(); openExperiment('journey', { from: { kind: 'exp', id: 'sculptor' } }); }, 1500, { view: true });
        });
        this.onSetting(ctx);
      },
    ...
    // ---------- FOCUS ----------
    {
      ...
      stop() { engine.resetMasterShape(); focus.setParam('soundTouch', false); focus.restoreVisual(); }, keepsSound: true,   /* the borrowed activity visual hands back to the user's own visual */
    },
    {
      ...
      finish(ctx) { if (ctx.s.fade) { engine.setTimer(0.1, true); app.toast('Fading out. Rest well.'); } else app.toast('Journey finished — the last sound stays on.'); },   // natural end and Finish
      ...
      start(ctx) {
        ...
        focus.setParam('dim', 0); focus.setParam('slow', 0); focus.setParam('time', ctx.s.j === 'ocean' ? 0.05 : 0.5); focus.setVisual(J[0].visual, { remember: false }); focus.setReturn('lab'); focus.enterFocus();
        runJourney(J, { crossfade: Math.min(120, u * 25), visuals: true, onEnd: () => { this.finish(ctx); stopRunning(null, true); } });
      },
      stop() { engine.resetMasterShape(); focus.setParam('dim', 0); focus.setParam('slow', 0); focus.setParam('time', 0.5); focus.restoreVisual(); }, keepsSound: true,
    },
    // ---------- SESSIONS ----------
    {
    ...
    },
    {
      ...
      buildUI(ctx, host) {
        ...
        const saveSession = () => { if (!engine.activeList().length) return app.toast('Start the session first, then save it.'); const ok = app.saveCurrentMix(`My ${ctx.s.doing} session`, { visual: focus.currentVisual(), motion: store.get('motion', 'low'), source: 'session' }); if (ok) app.toast('Session saved to My Saved Sessions', 3400); };
        render();
      },
      ...
      async start(ctx) {
        ...
        if (s.vis !== 'no') { const v = s.vis === 'surprise' ? VISUALS[Math.floor(Math.random() * VISUALS.length)].id : s.doing === 'sleep' ? (store.get('focus:userVisual') || 'underwater') : nature === 'rain' ? 'rainwindow' : nature === 'ocean' ? 'ocean' : nature === 'forest' ? 'forest' : (store.get('focus:userVisual') || 'underwater'); focus.setVisual(v, { remember: false }); focus.setReturn('lab'); focus.enterFocus(); } else if (s.doing === 'sleep') app.showView('sleep');
        app.toast(s.doing === 'sleep' ? 'Sleep session: 60-minute timer with gentle fade.' : 'Session ready — adjust anything you like.');
      },
      stop() { engine.setVariation(0); engine.resetMasterShape(); focus.restoreVisual(); }, keepsSound: true,
    },
  ];
  ...
  // Done: the user has finished with this experiment — leave for wherever it was opened from.
  function labDone(exp) {
    if (running && running.exp === exp) stopRunning();   // only if a new session started since; its natural completion
    feedbackOwed = false;
    const o = origins[exp.id] || { kind: 'list' };
    if (o.kind === 'result') { openExperiment('discovery', { keepOrigin: true }); return; }   // Fine-Tune → back to the result
    if (o.kind === 'exp' && byId[o.id]) { openExperiment(o.id, { keepOrigin: true }); return; }   // Sculptor → Journey → back to Sculptor
    const c = $('#lab-detail [data-close]'); if (c) c.click(); else { const p = $('#lab-detail'); p.hidden = true; p.innerHTML = ''; setFocusedExp(false); }
    if (o.kind === 'entry') { if (app.leaveLab) app.leaveLab(); return; }
  ...
  // ---------- cards & detail ----------
  const ctxs = {};
  // Where each experiment was opened from, so Done can return there:
  ...
  async function openSculptorRunning(params, nature, from) {
    ...
    if (ctxs.discovery && ctxs.discovery.finished && ctxs.discovery.result) setTimeout(() => {
      ...
      if (close && !$('[data-back-result]', p)) {
        ...
        close.after(b);
        b.addEventListener('click', () => { if (running) stopRunning(); openExperiment('discovery', { keepOrigin: true }); });
      }
    }, 100);
  }
  ...
  function showDiscoveryResult(ctx) {
    ...
    $('[data-r="tune"]', R).addEventListener('click', async () => {
      ...
      await openSculptorRunning(best.params, best.nature, { kind: 'result' });
    });
  ...
  function openExperiment(id, opts = {}) {
    ...
    panel.innerHTML = `<div class="lab-detail-inner"><button class="btn btn-ghost btn-sm" data-close>← Experiments</button>
      ...
      <div class="lab-run"><button class="btn btn-primary btn-lg" data-exp-start="${exp.id}">Start Experiment</button>${exp.id !== 'discovery' ? `<button class="btn btn-secondary" data-finish="${exp.id}" hidden>Finish</button>` : ''}<button class="btn btn-ghost" data-stop>Stop</button><button class="btn btn-ghost" data-reset>Reset</button></div>
    ...
    if (hh) {
      ...
      if (groups.length) {
        ...
        groups.forEach(([name, ids]) => { const g = document.createElement('div'); g.innerHTML = `<h3>${name}</h3><div class="chips"></div>`; ids.forEach(id => { const e = byId[id]; const b = document.createElement('button'); b.className = 'chip'; b.innerHTML = `<strong>${e.name}</strong><span>${e.cat}</span>`; b.addEventListener('click', () => openExperiment(id));
          ...
          if (name === 'Favourites') { const del = document.createElement('button'); del.type = 'button'; del.className = 'chip-del'; del.setAttribute('aria-label', 'Remove ' + e.name + ' from Favourites'); del.textContent = '×'; b.classList.add('chip-mine'); /* positions the × like the My Saved Sounds chips */ del.addEventListener('click', ev => { ev.stopPropagation(); const l = favs(); const i = l.indexOf(id); if (i >= 0) l.splice(i, 1); store.set('lab:favs', l); renderLists(); app.toast('Removed from Favourites'); }); b.appendChild(del); }
  ...
  const qe = new URLSearchParams(location.search).get('exp'); if (qe && byId[qe]) setTimeout(() => openExperiment(qe, { from: { kind: 'entry' } }), 120);
