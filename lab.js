...
(function () {
  ...
  function runJourney(segments, o = {}) {
    ...
    const apply = async (seg, fade) => { for (const id of union) { const m = seg.mix.find(x => x.id === id); if (m) { if (m.params) engine.setSculpt(m.params, id); if (!engine.isActive(id)) { await engine.startSound(id, 0.001); if (journeyTok !== me) { if (engine.isActive(id)) engine.stopSound(id); return; } } /* a start that lands after Stop is undone, not ramped */ engine.rampVolume(id, m.volume, fade); } else if (engine.isActive(id)) engine.rampVolume(id, 0.0001, fade); } if (seg.visual && o.visuals && seg.visual !== store.get('visual')) focus.crossfadeTo(seg.visual); if (seg.tone !== undefined) engine.setMasterTone(seg.tone, fade); if (o.visuals) { for (const key of ['dim', 'slow', 'time']) if (seg[key] !== undefined) rampParam(key, seg[key], fade); } };
  ...
  const EXPERIMENTS = [
    ...
    },
    {
      ...
      buildUI(ctx, host) {
        ...
        const saveSession = () => { if (!engine.activeList().length) return app.toast('Start the session first, then save it.'); const ok = app.saveCurrentMix(`My ${ctx.s.doing} session`, { visual: store.get('visual', 'underwater'), motion: store.get('motion', 'low'), source: 'session' }); if (ok) app.toast('Session saved to My Saved Sessions', 3400); };
        render();
      },
