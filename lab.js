...
(function () {
  ...
  function shapeLoop(now) { requestAnimationFrame(shapeLoop); if (document.hidden || now - shapeLast < 50) return; const labView = $('#view-lab'); if (!labView || labView.hidden) return; const dt = Math.min(0.05, (now - shapeLast) / 1000); shapeLast = now; shapeT += dt; const lv = engine.isPlaying ? Math.min(1, engine.getLevels(shapeSpec) * 6) : 0;
    liveShapes.forEach((get, c) => { if (!c.isConnected) { liveShapes.delete(c); return; } if (c.offsetParent === null) return; const r = c.getBoundingClientRect(); if (!r.width) return; const dpr = Math.min(devicePixelRatio || 1, 2); if (c.width !== Math.round(r.width * dpr)) { c.width = Math.round(r.width * dpr); c.height = Math.round(r.height * dpr); } const ctx = c.getContext('2d'); ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.clearRect(0, 0, r.width, r.height); const s = get(); SV.soundShape(ctx, r.width, r.height, s.p, shapeT * (s.speed || 1), s.live ? lv : 0.15, { scale: s.scale || 0.4 }); }); }
  ...
  const EXPERIMENTS = [
    ...
    },
    {
      ...
      buildUI(ctx, host) {
        ...
        const draw = () => { const w = c.width, h = c.height; const dark = document.documentElement.dataset.theme === 'dark'; const bg = cx.createLinearGradient(0, 0, 0, h); bg.addColorStop(0, dark ? '#0b1024' : '#f3f4fb'); bg.addColorStop(1, dark ? '#101a3a' : '#e4e9f6'); cx.fillStyle = bg; cx.fillRect(0, 0, w, h);
          cx.strokeStyle = dark ? 'rgba(140,170,255,.08)' : 'rgba(40,60,120,.07)'; cx.lineWidth = 1; for (let i = 1; i < 6; i++) { cx.beginPath(); cx.moveTo(0, h * i / 6); cx.lineTo(w, h * i / 6); cx.stroke(); } for (let i = 1; i < 8; i++) { cx.beginPath(); cx.moveTo(w * i / 8, 0); cx.lineTo(w * i / 8, h); cx.stroke(); }
          ...
          const g = cx.createLinearGradient(0, 0, w, 0); g.addColorStop(0, dark ? 'rgba(116,210,226,0.55)' : 'rgba(95,180,201,0.5)'); g.addColorStop(1, dark ? 'rgba(138,166,255,0.55)' : 'rgba(63,99,224,0.45)'); cx.fillStyle = g; cx.fill(); cx.strokeStyle = dark ? 'rgba(255,255,255,.85)' : 'rgba(20,30,60,.7)'; cx.lineWidth = 2.5; cx.lineJoin = 'round'; cx.stroke();
          cx.fillStyle = dark ? 'rgba(200,215,255,.55)' : 'rgba(40,60,120,.55)'; cx.font = '600 11px Manrope, sans-serif'; cx.fillText('LOW', 10, h - 14); cx.textAlign = 'right'; cx.fillText('HIGH', w - 10, h - 14); cx.textAlign = 'left';
