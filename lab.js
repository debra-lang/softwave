...
(function () {
  ...
  async function openSculptorRunning(params, nature) {
    ...
    if (ctxs.discovery && ctxs.discovery.finished && ctxs.discovery.result) setTimeout(() => {
      ...
      const fv = $('[data-fav]', p); if (fv) fv.hidden = true;   // pins the experiment, not this sound - misleading here
  ...
  function openExperiment(id) {
    ...
    $('[data-fav]', panel).addEventListener('click', e => { const l = favs(); const i = l.indexOf(exp.id); if (i >= 0) l.splice(i, 1); else l.push(exp.id); store.set('lab:favs', l); const on = i < 0; e.currentTarget.setAttribute('aria-pressed', on); e.currentTarget.textContent = on ? '★ Favourite experiment' : '☆ Favourite experiment'; app.toast(on ? '★ Added to Favourites — find it in the Favourites group on the Experiments page.' : 'Removed from Favourites.', 3600); renderLists(); });
