...
(function () {
  ...
  function runJourney(segments, o = {}) {
    ...
    const apply = async (seg, fade) => { for (const id of union) { const m = seg.mix.find(x => x.id === id); if (m) { if (m.params) engine.setSculpt(m.params, id); if (!engine.isActive(id)) await engine.startSound(id, 0.001); engine.rampVolume(id, m.volume, fade); } else if (engine.isActive(id)) engine.rampVolume(id, 0.0001, fade); } if (seg.visual && o.visuals) focus.crossfadeTo(seg.visual); if (seg.tone !== undefined) engine.setMasterTone(seg.tone, fade); };
  ...
  function learn(winner, nature) { const p = prefs(); p.n++; DIMS.forEach(d => { p.sum[d] = (p.sum[d] || 0) + winner[d]; }); if (nature) p.natures[nature] = (p.natures[nature] || 0) + 1; store.set('lab:prefs2', p); }
  ...
  function profile() {
    ...
    if (pp) { lines.push(pp.warm < -0.15 ? 'warmer sounds' : pp.warm > 0.15 ? 'brighter sounds' : 'a neutral tone'); lines.push(pp.colour < 0.4 ? 'lower-frequency noise' : pp.colour > 0.6 ? 'higher-frequency noise' : 'balanced noise'); lines.push(pp.moving > 0.25 || pp.mod > 0.25 ? 'gentle movement' : 'steady sound'); if (pp.width > 0.55) lines.push('a wide, spacious sound'); if (pp.smooth > 0.2) lines.push('a little texture'); if (pp.soft < -0.2) lines.push('softer edges'); }
    const nat = Object.entries(p.natures).sort((a, b) => b[1] - a[1])[0]; if (nat && nat[0] !== 'none') lines.push(`${NAME(nat[0]).toLowerCase()} textures`);
    ...
    const motion = store.get('motion', 'low'); lines.push(motion === 'still' || motion === 'low' ? 'low visual motion' : 'more visual motion'); lines.push(document.documentElement.dataset.theme === 'dark' ? 'darker visuals' : 'lighter visuals');
  ...
  function renderProfile() {
    ...
    el.innerHTML = `${pr.rounds ? `<p>You tend to prefer:</p><ul class="bullets">${pr.lines.map(l => `<li>${l}</li>`).join('')}</ul>` : `<p>Not much known yet. Run <strong>Sound Discovery</strong> and your preferences will appear here.</p><ul class="bullets">${pr.lines.slice(-2).map(l => `<li>${l}</li>`).join('')}</ul>`}
      <div class="btn-row"><button class="btn btn-primary btn-sm" data-p="sound" ${pr.rounds ? '' : 'disabled'}>Create sound from my profile</button><button class="btn btn-secondary btn-sm" data-p="sleep" ${pr.rounds ? '' : 'disabled'}>Build sleep session</button><button class="btn btn-secondary btn-sm" data-p="visual">Choose visual for my profile</button><button class="btn btn-ghost btn-sm" data-p="explore">Explore experiments</button></div>
      <p class="muted small">A preference profile, not a hearing profile. Built only from your own taps; stored only on this device. <button class="btn btn-ghost btn-sm" data-p="clear">Clear</button></p>`;
    $('[data-p="sound"]', el).addEventListener('click', async () => { safeMaster(); await engine.loadMix(soundMix({ params: pr.pp, nature: pr.nature, natureVol: 0.3 })); app.toast('Playing a sound built from your profile — fine-tune it in Sound Sculptor.'); store.set('lab:settings:sculptor', sculptSettingsFrom(pr.pp, pr.nature)); delete ctxs.sculptor; openExperiment('sculptor'); });
    ...
    $('[data-p="visual"]', el).addEventListener('click', () => { focus.setVisual(visualForProfile()); focus.enterFocus(); });
    $('[data-p="explore"]', el).addEventListener('click', () => $('#thelab').scrollIntoView({ behavior: 'smooth' }));
    $('[data-p="clear"]', el).addEventListener('click', () => { ['lab:prefs2', 'lab:feedback', 'playcounts'].forEach(k => store.del(k)); renderProfile(); renderLists(); app.toast('Profile cleared'); });
  }
  ...
  const EXPERIMENTS = [
      ...
      async start(ctx) {
        ...
        const finish = async () => {
          ...
          $('[data-result]', host).innerHTML = `<div class="card lab-result disc-result"><div class="label-sm">Your preferred sound</div><h3>${describe(best.params, best.nature)}</h3><p class="muted small">This is what you chose most often. It is a preference, not a measurement of your hearing.</p>
            <div class="btn-row"><button class="btn btn-primary" data-r="listen">Listen</button><button class="btn btn-secondary" data-r="tune">Fine tune</button><button class="btn btn-secondary" data-r="save">Save</button><button class="btn btn-ghost" data-r="fav">Add to favourites</button><button class="btn btn-ghost" data-r="sleep">Use for sleep</button><button class="btn btn-ghost" data-r="visual">Add visual</button><button class="btn btn-ghost" data-r="again">Try another discovery</button></div><div data-saveform></div></div>`;
    ...
    },
    {
      ...
      buildUI(ctx, host) {
        host.innerHTML = `<canvas class="paint-canvas" width="720" height="240" aria-label="Frequency painting canvas. Draw to shape the sound."></canvas><div class="scale"><span>60 Hz</span><span>500</span><span>2k</span><span>6k</span><span>14 kHz</span></div>
          <div class="btn-row"><button class="btn btn-secondary btn-sm" data-act="paint" aria-pressed="true">Paint</button><button class="btn btn-ghost btn-sm" data-act="reduce" aria-pressed="false">Reduce</button><button class="btn btn-ghost btn-sm" data-act="smooth">Smooth</button><button class="btn btn-ghost btn-sm" data-act="undo">Undo</button><button class="btn btn-ghost btn-sm" data-act="reset">Reset</button><button class="btn btn-ghost btn-sm" data-act="random">Randomize</button><button class="btn btn-ghost btn-sm" data-act="preview">Preview</button><button class="btn btn-secondary btn-sm" data-act="save">Save</button></div>
    ...
    },
    {
      ...
      buildUI(ctx, host) { host.innerHTML = `<p class="muted" data-desc></p><div class="btn-row"><button class="btn btn-secondary btn-sm" data-save>Save this sound</button></div><div data-saveform></div>`; $('[data-save]', host).addEventListener('click', () => saveSoundForm($('[data-saveform]', host), { type: 'sculpt', params: this.params(ctx), nature: ctx.s.nature, natureVol: 0.35, name: 'My sculpted sound' })); this.onSetting(ctx); },
    ...
    },
    {
      ...
      start(ctx) {
        ...
        const J = {
          ocean: [{ min: u, label: 'ocean', visual: 'ocean', mix: [{ id: 'ocean', volume: 0.6 }, { id: 'brown', volume: 0.2 }] }, { min: u, label: 'sunset', visual: 'ocean', mix: [{ id: 'ocean', volume: 0.55 }, { id: 'brown', volume: 0.3 }], tone: 9000 }, { min: u, label: 'darker water', visual: 'underwater', mix: [{ id: 'ocean', volume: 0.45 }, { id: 'brown', volume: 0.35 }], tone: 6000 }, { min: u, label: 'stars', visual: 'nightsky', mix: [{ id: 'ocean', volume: 0.3 }, { id: 'brown', volume: 0.35 }, { id: 'night', volume: 0.2 }], tone: 5000 }, { min: u, label: 'softer', visual: 'nightsky', mix: [{ id: 'brown', volume: 0.3 }, { id: 'night', volume: 0.15 }], tone: 4000 }],
          rain: [{ min: u, label: 'rain on the window', visual: 'rainwindow', mix: [{ id: 'rain', volume: 0.55 }, { id: 'pink', volume: 0.2 }] }, { min: u, label: 'darker room', visual: 'rainwindow', mix: [{ id: 'rain', volume: 0.5 }, { id: 'brown', volume: 0.25 }], tone: 8000 }, { min: u, label: 'slower rain', visual: 'particles', mix: [{ id: 'rain', volume: 0.3 }, { id: 'brown', volume: 0.35 }], tone: 6000 }, { min: u, label: 'soft noise', visual: 'softlight', mix: [{ id: 'brown', volume: 0.4 }, { id: 'rain', volume: 0.12 }], tone: 5000 }, { min: u, label: 'night', visual: 'nightsky', mix: [{ id: 'brown', volume: 0.3 }], tone: 4000 }],
          forest: [{ min: u, label: 'forest', visual: 'forest', mix: [{ id: 'forest', volume: 0.55 }, { id: 'pink', volume: 0.2 }] }, { min: u, label: 'sunset', visual: 'clouds', mix: [{ id: 'forest', volume: 0.45 }, { id: 'wind', volume: 0.3 }], tone: 9000 }, { min: u, label: 'evening', visual: 'forest', mix: [{ id: 'wind', volume: 0.35 }, { id: 'brown', volume: 0.25 }], tone: 6000 }, { min: u, label: 'night ambience', visual: 'nightsky', mix: [{ id: 'night', volume: 0.35 }, { id: 'brown', volume: 0.3 }], tone: 5000 }, { min: u, label: 'softer', visual: 'nightsky', mix: [{ id: 'brown', volume: 0.3 }, { id: 'night', volume: 0.15 }], tone: 4000 }],
        }[ctx.s.j];
        focus.setVisual(J[0].visual); focus.enterFocus();
    ...
    // ---------- SESSIONS ----------
    {
      ...
      settings: [{ key: 'len', label: 'Length', type: 'buttons', options: [[10, '10 min'], [20, '20 min'], [30, '30 min'], [60, '60 min'], [0, 'Continuous']] }, { key: 'var', label: 'Variation', type: 'buttons', options: [['stable', 'Very stable'], ['gentle', 'Gentle changes'], ['dynamic', 'More dynamic']] }, { key: 'sleep', label: 'Fade toward sleep at the end', type: 'toggle' }],
      defaults: { len: 20, var: 'gentle', sleep: false },
      start(ctx) {
        safeMaster(); const L = ctx.s.len || 60; const u = L / 5; const k = ctx.s.var === 'stable' ? 0.4 : ctx.s.var === 'dynamic' ? 1.3 : 1;
        const segs = [
          { min: u, label: 'brown noise + rain', mix: [{ id: 'brown', volume: 0.55 }, { id: 'rain', volume: 0.35 * k }] },
          { min: u, label: 'less rain', mix: [{ id: 'brown', volume: 0.55 }, { id: 'rain', volume: 0.15 * k }] },
          { min: u, label: 'ocean appears', mix: [{ id: 'brown', volume: 0.45 }, { id: 'ocean', volume: 0.4 * k }, { id: 'rain', volume: 0.05 }] },
          { min: u, label: 'warmer', mix: [{ id: 'brown', volume: 0.5 }, { id: 'ocean', volume: 0.3 * k }], tone: 6000 },
          { min: u, label: 'simpler', mix: [{ id: 'brown', volume: ctx.s.sleep ? 0.3 : 0.45 }, { id: 'ocean', volume: 0.12 * k }], tone: 5000 },
        ];
    ...
    },
    {
      ...
      defaults: { doing: 'relax', pref: 'unsure', vis: 'yes', var: 'some' },
      async start(ctx) {
  ...
  function card(exp) {
    ...
    el.innerHTML = `<div class="lab-card-head"><div><div class="lab-cat">${exp.cat}${exp.premium ? ' · <span class="tag tag-prem">Premium preview</span>' : ''}</div><h3>${exp.name}</h3></div><span class="ev ev-${exp.evidence}">${EV[exp.evidence]}</span></div><p class="lab-what">${exp.what}</p><div class="lab-actions"><button class="btn btn-primary" data-open="${exp.id}">Open</button>${f.rating ? `<span class="muted small">You said: ${f.rating === 'helpful' ? 'Helpful' : f.rating === 'not' ? 'Not for me' : 'Neutral'}</span>` : ''}${favs().includes(exp.id) ? '<span class="tag">★ Favourite</span>' : ''}</div>`;
