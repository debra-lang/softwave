...
(function () {
  ...
  function soundObject(ctx, w, h, s, t, lv, holder) {
    ...
    const red = F.reduced();
    const o = { cx, cy, R, W: w, H: h, t: t * (s.speed || 1), life, alive: s.live ? 1 : 0.35, level: lv, low: lv * 0.6, vol: 0.4, P, tint, dark, e: 0, rawMorph: 0, grow: 1, reduce: red };
  ...
  function shapeLoop(now) { requestAnimationFrame(shapeLoop); if (document.hidden || now - shapeLast < 50) return; const labView = $('#view-lab'); if (!labView || labView.hidden) return; const dt = Math.min(0.05, (now - shapeLast) / 1000); shapeLast = now; shapeT += dt; const lv = engine.isPlaying ? Math.min(1, engine.getLevels(shapeSpec) * 6) : 0;
    liveShapes.forEach((get, c) => { if (!c.isConnected) { liveShapes.delete(c); return; } if (c.offsetParent === null) return; const r = c.getBoundingClientRect(); if (!r.width) return; const dpr = Math.min(devicePixelRatio || 1, 2); if (c.width !== Math.round(r.width * dpr)) { c.width = Math.round(r.width * dpr); c.height = Math.round(r.height * dpr); } const ctx = c.getContext('2d'); ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.clearRect(0, 0, r.width, r.height); const s = get(); if (s.preview) { const P = SF() ? SF().PERSONA.brown : null; const tint = isDark() ? [190, 200, 215] : [70, 90, 120]; ctx.fillStyle = isDark() ? 'rgba(14,18,28,.9)' : 'rgba(236,232,224,.9)'; ctx.fillRect(0, 0, r.width, r.height); PREVIEW[s.preview] && PREVIEW[s.preview](ctx, r.width, r.height, shapeT, isDark(), s.tint || tint); } else if (s.field) labField(ctx, r.width, r.height, shapeT); else soundObject(ctx, r.width, r.height, s, shapeT, s.live ? lv : 0.15); }); }
