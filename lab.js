...
(function () {
  ...
  function shapeLoop(now) { requestAnimationFrame(shapeLoop); if (document.hidden || now - shapeLast < 33) return; const dt = Math.min(0.05, (now - shapeLast) / 1000); shapeLast = now; shapeT += dt; const lv = engine.isPlaying ? Math.min(1, engine.getLevels(shapeSpec) * 6) : 0;
