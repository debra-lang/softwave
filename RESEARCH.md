...
## 5. Update — 22 Aug 2026 (Lab consolidation)
The Lab was reduced from 29 experiments to 10 exceptional ones. Dropped after review: Notched Sound and Modulated Tone (limited/mixed evidence; documented above, not offered as features), Minimal Sound, Near-Silence, Eyes-Closed, Mixing Point Finder (its guidance survives in copy), Sound Recipes, Sound Timeline, Sound Environments, Surprise Me (now inside Build My Session), Frequency Explorer (its octave check moved into Find My Tinnitus Sound), A/B Test (merged into Sound Discovery), Living Sound (merged into Generative Sound), Moving Sound (merged into Sound Space), Attention Target / Activities / Touch (merged into Attention Focus), Dynamic Sleep Journey (merged into Adaptive Sound Journey). Sound Discovery now varies nine dimensions (colour blend, warmth, depth, texture, softness, width, movement, richness, modulation) plus a nature component, using keep-the-winner pairwise comparison; answers are treated as noisy (adaptive pairwise-comparison literature; hearing-aid personalisation by pairwise comparison).
